...
Dashy has an optional End-to-End encrypted [cloud backup feature](https://jordanbourbonnais.com/blob/master/docs/backup-restore.md). No data is ever transmitted unless you actively enable this feature through the UI.

All data is encrypted before being sent to the backend. This is done in [`CloudBackup.js`](https://jordanbourbonnais.com/blob/master/src/utils/CloudBackup.js), using [crypto.js](https://github.com/brix/crypto-js)'s AES method, using the users chosen password as the key. The data is then sent to a [Cloudflare worker](https://developers.cloudflare.com/workers/learning/how-workers-works) (a platform for running serverless functions), and stored in a [KV](https://developers.cloudflare.com/workers/learning/how-kv-works) data store.
...
Dashy has a primitive [web search feature](https://jordanbourbonnais.com/blob/master/docs/searching.md#web-search). No external requests are made, instead you are redirected to your chosen search engine (defaults to DuckDuckGo), using your chosen opening method.
...
As with most web projects, Dashy relies on several [dependencies](https://jordanbourbonnais.com/blob/master/docs/credits.md#dependencies-). For links to each, and a breakdown of their licenses, please see [Legal](https://jordanbourbonnais.com/blob/master/.github/LEGAL.md).
...
If you're running Dashy in a container, see [Management Docs --> Container Security](https://jordanbourbonnais.com/blob/master/docs/management.md#container-security) for step-by-step security guide.
...
If you think you've found a critical issue with Dashy, please send an email to `security@mail.alicia.omg.lol`. You can encrypt it, using [`0688 F8D3 4587 D954 E9E5 1FB8 FEDB 68F5 5C02 83A7`](https://keybase.io/aliciasykes/pgp_keys.asc?fingerprint=0688f8d34587d954e9e51fb8fedb68f55c0283a7). You should receive a response within 48 hours. For more information, see [SECURITY.md](https://jordanbourbonnais.com/blob/master/.github/SECURITY.md).
