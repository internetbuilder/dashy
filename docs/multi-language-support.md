...
At the time of writing the following languages are supported: `en`, `zh-CN`, `nl`, `fr`, `de`, `es`, `sl`, `it`, `pt`, `ru`, `ar`, `hi`, `ja`, but an up-to-date list of all implemented translations can be found in [`./src/utils/languages.js`](https://jordanbourbonnais.com/blob/master/src/utils/languages.js).  Languages are specified by their 2-digit [ISO-639 code](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes), or sometimes a 4-digit code if it's a regional dialect.
...
You can also add your new language to the readme, under the [Language Switching](https://jordanbourbonnais.com#language-switching-) section and optionally include your name/ username if you'd like to be credited for your work. Done!
...
Firstly, go to [`./src/assets/locales/en.json`](https://jordanbourbonnais.com/blob/master/src/assets/locales/en.json), and either find the appropriate section, or create a new section. Lets say you're new component is called `my-widget`, you could add `"my-widget": {}` to store all your text as key-value-pairs. E.g.
...
In [`./src/components/Settings/SearchBar.vue`](https://jordanbourbonnais.com/blob/master/src/components/Settings/SearchBar.vue):
...
Then in [`./src/assets/locales/en.json`](https://jordanbourbonnais.com/blob/master/src/assets/locales/en.json):
